...
describe('PRandom', () => {
  it('should generate deterministic numbers', () => {
    ...
    expect(numbers).toEqual([
      0.9872818551957607, 0.34880331158638, 0.5631933622062206, 0.9990169629454613,
      0.8291510976850986, 0.13247808441519737, 0.8254958242177963, 0.8386827223002911,
      0.11304118484258652, 0.058928895741701126,
    ]);
  });
  ...
  it('should generate deterministic numbers with different variant', () => {
    ...
    for (let i = 0; i < 10; i++) numbers[i] = random.next();
    expect(numbers).toEqual([
      0.981050293892622, 0.2517034336924553, 0.4143013134598732, 0.5354671590030193,
      0.17058493942022324, 0.46943316981196404, 0.16233910247683525, 0.2525038607418537,
      0.6119818463921547, 0.3286912925541401,
    ]);
  });
  ...
  it('should cover the entire alphanumeric set', () => {
    ...
    expect(Array.from(characters).sort().join('')).toEqual('0123456789abcdefghijklmnopqrstuvwxyz');
  });
});
